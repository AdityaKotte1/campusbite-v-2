...
# --- Python packages (isolated venv — avoids clashing with system Python / PEP 668) ---
python3 -m venv /opt/munchadda-kiosk/venv
/opt/munchadda-kiosk/venv/bin/pip install --upgrade pip setuptools wheel
/opt/munchadda-kiosk/venv/bin/pip install -r /opt/munchadda-kiosk/requirements.txt

# --- Directory structure ---
mkdir -p /opt/munchadda-kiosk/logs
mkdir -p /opt/munchadda-kiosk/db
mkdir -p /opt/munchadda-kiosk/assets/sounds
chown -R kiosk:kiosk /opt/munchadda-kiosk

# Secure sensitive directories (not world-readable)
chmod 0700 /opt/munchadda-kiosk/logs
chmod 0700 /opt/munchadda-kiosk/db
chmod 0700 /opt/munchadda-kiosk/config

# Create secrets directory outside the app folder
mkdir -p /etc/munchadda-kiosk
chmod 0700 /etc/munchadda-kiosk
chown root:root /etc/munchadda-kiosk

echo "IMPORTANT: Copy kiosk/config/secrets.env.example to /etc/munchadda-kiosk/secrets.env"
echo "           Then fill in your API key and DB encryption key"
echo "           Run: chmod 600 /etc/munchadda-kiosk/secrets.env"
...
# --- systemd service ---
cp /opt/munchadda-kiosk/systemd/munchadda-kiosk.service /etc/systemd/system/
systemctl daemon-reload
systemctl enable munchadda-kiosk.service

# --- Auto-update cron (daily at 3 AM) ---
crontab -u kiosk - << 'EOF'
0 3 * * * /opt/munchadda-kiosk/scripts/update.sh >> /opt/munchadda-kiosk/logs/update.log 2>&1
EOF

# --- UFW firewall ---
ufw default deny outgoing
ufw default deny incoming
# HTTPS to MunchAdda server
...
echo ""
echo "Next steps:"
echo "  1. Copy config:  cp /opt/munchadda-kiosk/config/kiosk.yaml.example /opt/munchadda-kiosk/config/kiosk.yaml"
echo "  2. Edit config:  nano /opt/munchadda-kiosk/config/kiosk.yaml"
echo "     (set server URL, kiosk ID, API key, canteen ID)"
echo "  3. Find printer: lsusb   (note VID:PID and update printer section)"
echo "  4. Start:        systemctl start munchadda-kiosk.service"
echo "  5. Check logs:   tail -f /opt/munchadda-kiosk/logs/kiosk.log"
